...
if __name__ == "__main__":
    ...
    response = response_url(url)
    soup = BeautifulSoup(response, 'lxml')
    hot_item = soup.find_all('section',{'class':{'HotItem'}})
    data = []
    for item in hot_item:
        content = item.find_all('div',{'class':'HotItem-content'})
        data.append((content[0].contents[0]['title'],content[0].contents[0]['href']))        
    df_data = pd.DataFrame(data=data, columns=['Title', 'Url'])
    print(df_data)